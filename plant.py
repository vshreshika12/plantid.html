...
convolution layer
 ...
 for i in range(len(out_pool_size)):
    ...
    max_pool = tf.nn.max_pool(new_previous_conv,ksize=[1,h_size, h_size, 1],strides=[1,h_strd, w_strd,1],padding='SAME')
 if (i == 0):
...
while it < TRAINING_ITERATIONS:
    ...
    with graph.as_default():
        ...
        if len(y_train) < 50:
            ...
            input_queue_train = tf.train.slice_input_producer([x_train, y_train],num_epochs=None,shuffle=True)
            x_train, y_train = read_images_from_disk(input_queue_train)
            print(size_cluster_keys[it%len(size_cluster_keys)])
            x_train = tf.image.resize_images(x_train,[size_cluster_keys[it%len(size_cluster_keys)][1]/2,size_cluster_keys[it%len(size_cluster_keys)][0]/2],method=1, align_corners=False)
 ...
 # conv5
                ...
                int(conv5.get_shape()[0]),[int(conv5.get_shape()[1]), int(conv5.get_shape()[2])],out_pool_size)
 # fc6
 ...
 # fc8
            ...
            logits = model(x)
            cross_entropy = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=logits, abels=y_))
 ...
 # evaluation
            ...
            saver = tf.train.Saver({v.op.name: v for v in [conv1W, conv1b,conv2W, conv2b,conv3W, conv3b,conv4W, conv4b,conv5W, conv5b,fc6W, fc6b,fc7W, fc7b,fc8W, fc8b]})
...
def get_result(identification_id):
    params = {"api_key": "G2AldiuJBTtXiRbopdb7K580","plant_language": "en","plant_details": ["common_names", "url",
            "name_authority,",
            "wiki_description",
            "taxonomy",
            "synonyms"]}
